...
router.post('/select', async (req, res) => {
  ...
  // Store the selected ingredients in MongoDB
  try {
    const selectedItems = selectedIngredients.map(name => ({ name }));
    await Ingredient.insertMany(selectedItems);
    res.status(201).json({ message: "Ingredients saved successfully", selectedIngredients });
  } catch (error) {
    res.status(500).json({ message: "Error saving ingredients", error });
  }
});
...
// ✅ Route to select ingredients, avoiding duplicates
router.post('/select', async (req, res) => {
  const { selectedIngredients } = req.body;
